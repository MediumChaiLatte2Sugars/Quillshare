          ...
          {user && userObject ? <ProfileSidebar setMode={setMode} mode={mode} user={userObject} viewerIsSelf={user.email === userObject.email} currentViewer={currentViewer} handleFollow={handleFollow} handleUnFollow={handleUnFollow} isFollowed={isFollowed}/> : <Skeleton variant="text" sx={{ fontSize: '2rem' }} animation="wave" />}
           ...
           <Box flex={4} p={{ xs: 0, md: 2 }}>
            ...
            { userStories ? userStories.map((story) => { 
                ...
                return <SavedStoryList key={updatedStory.id} author={updatedStory.user_id} currentViewer={props.otherUser && currentViewer ? currentViewer.id : userObject.id} story={{created_at: updatedStory.created_at, title: updatedStory.title, content: updatedStory.content, id: updatedStory.id, unique_id: updatedStory.unique_id, status: updatedStory.status}}/>;
